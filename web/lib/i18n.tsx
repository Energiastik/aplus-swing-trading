...
const DICT = {
  ru: {
    ...
    rrg_axes_note: "Ось X — JdK RS-Ratio (сила против SPY), ось Y — JdK RS-Momentum (ускорение). Наведите на хвост, чтобы выделить сектор; кликните по метке ниже, чтобы скрыть/показать его.",
    rrg_unavailable: "Данные RRG (относительная ротация) за этот запуск недоступны.",
    rrg_only_sectors: "Только 11 секторов",
    rrg_add_themes: "+ 14 тем",
    rrg_week_label: "Неделя:",
    rrg_to_current_week: "К текущей неделе →",
    rrg_show_all: "Показать все",
    rrg_hide_all: "Скрыть все",
    rrg_default_filter_note: "По умолчанию показаны только сектора/темы в квадрантах «Улучшение» и «Лидеры» (ускоряющиеся) — остальное скрыто, чтобы не создавать визуальный шум. Кликните метку ниже или «Показать все», чтобы вернуть скрытые.",
  ...
  en: {
    ...
    rrg_title: "Relative Rotation Graph (JdK RS-Ratio / RS-Momentum)",
    rrg_axes_note: "X axis — JdK RS-Ratio (strength vs. SPY), Y axis — JdK RS-Momentum (acceleration). Hover a tail to highlight one sector; click a label below to hide/show it.",
    rrg_unavailable: "RRG (relative rotation) data isn't available for this run.",
    rrg_only_sectors: "11 sectors only",
    rrg_add_themes: "+ 14 themes",
    rrg_week_label: "Week:",
    rrg_to_current_week: "Jump to current week →",
    rrg_show_all: "Show all",
    rrg_hide_all: "Hide all",
    rrg_default_filter_note: "Only names in the Improving / Leading quadrants (still accelerating) are shown by default — the rest are hidden to cut visual noise. Click a label below, or \"Show all\", to bring hidden ones back.",
