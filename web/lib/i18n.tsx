...
const DICT = {
  ru: {
    ...
    watchlist_subtitle: "Тикеры со статусом WAIT — стоит последить. Один тикер = последняя проверка. Добавить новый тикер можно только через /add в Telegram-боте. BUY и PASS отправляются в Telegram, но не остаются в этом списке.",
    watchlist_justification: "Обоснование",
    watchlist_empty: "Нет тикеров со статусом WAIT прямо сейчас. Отправьте /add TICKER боту в Telegram.",
  ...
  en: {
    ...
    watchlist_subtitle: "Tickers currently WAIT — worth watching. One row per ticker (latest check wins). New tickers can only be added via /add in the Telegram bot. BUY and PASS results are sent to Telegram but don't stay listed here.",
    watchlist_justification: "Justification",
    watchlist_empty: "No WAIT tickers right now. Send /add TICKER to the Telegram bot.",
